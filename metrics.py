...
def evaluate(df, prefix, top_k_list=[1, 4, 5, 10, 20, 50, 100]):
    metric = {}
    df[['sorted_score', 'sorted_candidate_collection', 'sorted_candidate_key']] = df.apply(sorting_collections_by_score,
                                                                                           axis=1,
                                                                                           result_type='expand')
    ...
    for top_k in tqdm(top_k_list):
        metric[f'{prefix}/Recall@{top_k}'] = df[f'Recall@{top_k}'].sum() / df.shape[0]
        metric[f'{prefix}/MRR@{top_k}'] = df[f'MRR@{top_k}'].sum() / df.shape[0]
    return metric, df
...
def sorting_collections_by_score(df):
    ...
    key_list = df['c_key_list']
    sorted_indices = np.argsort(score)[::-1]
    score, collection_list, key_list = np.array(score), np.array(collection_list), np.array(key_list)
    return score[sorted_indices], collection_list[sorted_indices], key_list[sorted_indices]
